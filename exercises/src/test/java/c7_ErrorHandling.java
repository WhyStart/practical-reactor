...
public class c7_ErrorHandling extends ErrorHandlingBase {
    ...
    public void error_reporter() {
        //todo: feel free to change code as you need
        Flux<String> messages = messageNode().onErrorResume(t -> this.errorReportService(t).then(Mono.error(t)));
    ...
    public void resilience() {
        ...
        Flux<String> content = getFilesContent()
                .flatMap(mapper -> mapper.doOnError(error -> System.out.println("read file content error:" + error.getMessage()))
                .onErrorResume(err -> Mono.empty())); //start from here
